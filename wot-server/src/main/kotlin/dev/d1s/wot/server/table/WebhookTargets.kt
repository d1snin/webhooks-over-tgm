...
object WebhookTargets : Table<WebhookTarget>("webhook_target") {

    val webhookId = text("webhook_id").references(Webhooks) {
        it.webhook
    }

    val targetId = text("target_id").references(Targets) {
        it.target
    }
}